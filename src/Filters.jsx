...
export function GeoFilter({ selectedRegion, setSelectedRegion }) {
  ...
  return (
    <div className="flex flex-col">
      <label htmlFor="region" className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
        {t("filters.geographical_region")}
      </label>
      <select
        id="region"
...
export function GroupFilter({ selectedGroup, setSelectedGroup }) {
  const t = useTranslations();
  const tGroups = useTranslations("ideological_groups_display");
  return (
    <div className="flex flex-col">
      <label htmlFor="group" className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
        {t("filters.ideological_group")}
      </label>
      <select
        id="group"
        value={selectedGroup}
      ...
      >
        ...
        {IdeologicalGroups &&
          typeof IdeologicalGroups === "object" &&
          Object.keys(IdeologicalGroups).map((groupKey) => (
            <option key={groupKey} value={groupKey}>
              {tGroups.has(groupKey) ? tGroups(groupKey) : groupKey}
            </option>
          ))}
      </select>
    </div>
...
export default function Filters({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
      ...
      <div className="flex flex-col">
        <label htmlFor="sort-by" className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
          {t("filters.sort_by")}
        </label>
        <select
          id="sort-by"
