// Shared year parsing helpers (used by filtering/sorting across pages)
...
export const getStartYear = (yearsString) => {
  ...
  const bcAdMatch = str.match(
    new RegExp(
      `${prefixPattern}(\\d+)\\s*(bc|bce)\\s*[-–]\\s*${prefixPattern}(\\d+)(?:\\s*(ad|ce))?`,
      "i"
    )
  );
  ...
  let bcRangeMatch = str.match(
    new RegExp(
      `${prefixPattern}(\\d+)\\s*[-–]\\s*${prefixPattern}(\\d+)\\s*(bc|bce)`,
      "i"
    )
  );
  if (bcRangeMatch) {
  ...
  bcRangeMatch = str.match(
    new RegExp(
      `${prefixPattern}(bc|bce)\\s*(\\d+)\\s*[-–]\\s*${prefixPattern}(\\d+)`,
      "i"
    )
  );
  if (bcRangeMatch) {
  ...
  let singleBcYearMatch = str.match(
    new RegExp(`${prefixPattern}(\\d+)\\s*(bc|bce)`, "i")
  );
  if (singleBcYearMatch) {
    // Format: "470 BC"
    return -parseInt(singleBcYearMatch[1], 10);
  }
  singleBcYearMatch = str.match(
    new RegExp(`${prefixPattern}(bc|bce)\\s*(\\d+)`, "i")
  );
  if (singleBcYearMatch) {
    // Format: "BC 470" or "c. BC 470"
  ...
  let centuryBcMatch = str.match(
    new RegExp(
      `${prefixPattern}(\\d+)(st|nd|rd|th)\\s+century\\s+(bc|bce)`,
      "i"
    )
  );
  if (centuryBcMatch) {
    // Format: "6th century BC"
    const century = parseInt(centuryBcMatch[1], 10);
    return -(century * 100);
  }
  centuryBcMatch = str.match(
    new RegExp(
      `${prefixPattern}(bc|bce)\\s*(\\d+)(st|nd|rd|th)\\s+century`,
      "i"
    )
  );
  if (centuryBcMatch) {
    // Format: "BC 6th century"
  ...
  let centuryAdMatch = str.match(
    new RegExp(
      `${prefixPattern}(\\d+)(st|nd|rd|th)\\s+century(?:\\s*(ad|ce))?`,
      "i"
    )
  );
  if (centuryAdMatch) {
    // Format: "5th century AD" or "5th century"
    const century = parseInt(centuryAdMatch[1], 10);
    return (century - 1) * 100 + 1;
  }
  centuryAdMatch = str.match(
    new RegExp(
      `${prefixPattern}(ad|ce)\\s*(\\d+)(st|nd|rd|th)\\s+century`,
      "i"
    )
  );
  if (centuryAdMatch) {
    // Format: "AD 5th century"
  ...
  let adRangeMatch = str.match(
    new RegExp(
      `${prefixPattern}(\\d+)\\s*[-–]\\s*${prefixPattern}(\\d+)(?:\\s*(ad|ce))?`,
      "i"
    )
  );
  if (adRangeMatch) {
    // Format: "1775-1854 AD" or "1775-1854"
    return parseInt(adRangeMatch[1], 10); // First year in the range
  }
  adRangeMatch = str.match(
    new RegExp(
      `${prefixPattern}(ad|ce)\\s*(\\d+)\\s*[-–]\\s*${prefixPattern}(\\d+)`,
      "i"
    )
  );
  if (adRangeMatch) {
  ...
  let singleAdYearMatch = str.match(
    new RegExp(`${prefixPattern}(\\d+)(?:\\s*(ad|ce))?`, "i")
  );
  if (singleAdYearMatch) {
    // Format: "1950" or "1950 AD"
    return parseInt(singleAdYearMatch[1], 10);
  }
  singleAdYearMatch = str.match(
    new RegExp(`${prefixPattern}(ad|ce)\\s*(\\d+)`, "i")
  );
  if (singleAdYearMatch) {
    // Format: "AD 1950" or "c. AD 1950"
